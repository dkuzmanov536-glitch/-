...
- **Клиент** (таб „Профил“ → `#profile`): вход и **регистрация** (`/auth/v1/signup`). При регистрация
  се въвеждат и име, фамилия, телефон — записват се в `user_metadata` на Supabase Auth (полето `data`
  при signup) и се предпопълват автоматично в „Завършване на поръчката“. Сесията се пази в
  `localStorage` (`customer_session`, вкл. `first_name`/`last_name`/`phone`) и се подновява с refresh
