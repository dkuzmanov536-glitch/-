...
- `products` — id, name, price, `sale_price` (промоцена; ако е под price → намаление), category,
  image (емоджи или URL), stock, description, created_at
...
- `messages` — id, created_at, user_id, name, email, body, handled, `sender` ('customer'/'admin') —
  двупосочен чат клиент↔продавач през сайта. Гост може да прати еднопосочно; регистриран клиент
  вижда отговорите в „Профил“. RLS: всеки може да вмъкне (клиент само със своя user_id, админ за всеки),
  клиентът чете своя разговор (user_id = auth.uid()), админът чете/трие всичко. Админът отговаря в таб
  „Съобщения“ (нишки по клиент); нови съобщения се известяват по имейл чрез Formspree (`FORMSPREE_ENDPOINT`).
